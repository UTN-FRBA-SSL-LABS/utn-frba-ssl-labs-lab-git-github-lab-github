...
> R:Porque git revert no modifica el historial, sino que crea un nuevo commit que deshace los cambios del anterior.
En cambio, git reset --hard reescribe la historia: borra commits que ya están publicados.
Si otras personas (o GitHub) ya descargaron esos commits, esto causa conflictos, errores de sincronización y obliga a todos a forzar sus repos.
...
> R:Ambas implementaciones daban el mismo resultado para enteros:

(n % 2) == 0 usa el operador módulo y verifica si el resto de dividir por 2 es cero.
(n & 1) == 0 mira el último bit del número: si el bit menos significativo es 0, el número es par.

La diferencia real es que % es más intuitivo y claro para leer, mientras que & es una operación a nivel de bits, más técnica.
Elegir una u otra podría importar en contextos de bajo nivel o optimización extrema, donde las operaciones bitwise pueden ser apenas más rápidas, pero en código común se prefiere % por legibilidad y claridad.
...
> R:Hacer un solo commit diario mezcla muchos cambios distintos en un único punto del historial, lo que dificulta entender qué hizo cada modificación.
Si más adelante hay que revertir un cambio puntual, no se puede sin perder todo lo que se hizo ese día.
Además, los Pull Requests quedan muy grandes y difíciles de revisar porque juntan tareas que no están relacionadas entre sí.
Lo recomendable es hacer commits pequeños, atómicos y frecuentes, cada uno representando un cambio lógico claro.

**P4** — ¿Cuál es la diferencia entre `git fetch` y `git pull`? ¿Cuándo preferirías usar uno sobre el otro?

> R:git fetch descarga los cambios remotos, pero no modifica tu working directory ni tu branch actual; solo actualiza las referencias de Git.
git pull hace fetch + merge automáticamente, es decir, trae los cambios y los intenta integrar en tu branch actual.

**P5** — ¿Qué información debería tener la descripción de un Pull Request para que sea útil para quien lo revisa?

> R:La descripción debe explicar qué problema se resuelve o qué funcionalidad se agrega, cómo se verificó que funciona (pruebas, screenshots, comandos usados), y cualquier decisión de diseño relevante que ayude al reviewer a entender el contexto.
Esto facilita la revisión y evita preguntas innecesarias.
>>>>>>> origin/feature/conflicto-demo
